...
% Per equation (M.43) search for a special type of witness.

function [ value, witness, state ] = findOptimalWitness_3modes (G, N)

  % This is a specialized modification of (H.44), with condition (M.43)
  % imprinted in the structure of the witness matrix W.

  if not(N == 3)
    error('H.44.M.43 expects three modes and no less!')
  end
  ...
  % Per (M.43) we have constrain the structure of the witness. To avoid
  % additional constraints we imprint this onto the actual witness matrix.
  %
  % => Prohibit any correlation between the 1st and the 3rd system.
